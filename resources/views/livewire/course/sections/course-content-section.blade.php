            ...
            <div class="space-y-3">
                @foreach ($course->lessons as $lesson)
                    <div @class([
                        ...
                        @if ($action['is_enroll_action'])
                            {{-- Use a button or a link with wire:click to trigger enrollment --}}
                            <button
                                class="px-3 py-1 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 transition-colors"
                                type="button"
                                wire:click="$dispatchTo('course.sections.course-enrollment-section', 'trigger-enroll')"
                                wire:loading.attr="disabled"
                                x-data="{ isLoading: false }"
                                @click="isLoading = true"
                                @enrolled.window="isLoading = false"
                                @enrollment-failed.window="isLoading = false"
                            >
                                <span x-show="!isLoading">{{ $action['label'] }}</span>
                                <span x-show="isLoading">{{ __('Enrolling...') }}</span>
                            </button>
                        @else
