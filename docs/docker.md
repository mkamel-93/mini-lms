...
| `make build-project` | Install deps, migrate, seed, build assets |
...
|---------|-----------|----------|--------------------------------|
| PHP/App | `{APP_NAME}_php` | `172.18.10.11` | `make php-bash`                |
| Web/Nginx | `{APP_NAME}_web` | `172.18.10.12` | Port 80, 5173                  |
| MySQL | `{APP_NAME}_database` | `172.18.10.13` | Port 3306                      |
| phpMyAdmin | `{APP_NAME}_phpmyadmin` | `172.18.10.14` | Port 80                        |
| MailHog | `{APP_NAME}_mailhog` | `172.18.10.15` | Port 8025 (UI)                 |
| Redis | `{APP_NAME}_redis` | `172.18.10.16` | Port 6379                      |
| Selenium | `{APP_NAME}_selenium` | `172.18.10.17` | Port 4444, 5900 for NVC viewer |

This ensures reliable inter-container communication for services like:
- PHP connecting to MySQL (`DB_HOST=172.18.10.13` || `DB_HOST=mysql` service name)
- PHP connecting to Redis (`REDIS_HOST=172.18.10.16` || `REDIS_HOST=redis` service name)
- Laravel Dusk connecting to Selenium (`DUSK_DRIVER_URL=http://172.18.10.17:4444/wd/hub`)
